    ...
    while IFS= read -r line; do
        [[ "$line" =~ \"name\" ]] && {
            ...
            if _import_connection_from_json "$line"; then
                ((success++))
                log_info "✓ Connection $count imported"
            else
                ((failed++))
                log_warn "✗ Failed to import connection $count"
            fi
        }
    done < "$file"
...
_import_connections_csv() {
    ...
    while IFS=',' read -r name user host port pdb password; do
        ...
        sandbox conn add \
            ...
            ((success++))
            log_info "✓ Connection imported: $name"
        } || {
            ((failed++))
...
_validate_import_connections() {
    local file="$1"
    log_info "Validating connection imports..."
    ...
    log_info "Found $count connections to import"
}
...
case "$RESOURCE" in
    connections)
        log_info "Importing connections from $_file"
        if [[ "$_file" == *.json ]]; then
            _import_connections_json "$_file"
        elif [[ "$_file" == *.csv ]]; then
            _import_connections_csv "$_file"
        else
            log_error "Unsupported file format. Use .json or .csv"
            exit 1
        fi
        ;;
    config)
        log_info "Importing configuration from $_file"
        if [[ "$_file" == *.json ]]; then
        ...
        ;;
    all)
        log_info "Importing all resources from $_file"
        log_warn "Bulk import not yet implemented. Use 'connections' or 'config' resource"
        exit 1
        ;;
    *)
        log_error "Unknown resource: $RESOURCE. Use: connections|config|all"
        exit 1
        ;;
esac
